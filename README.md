...
| パス | 中身 |
| --- | --- |
| [`tools/ev-hoshu-hikaku/`](tools/ev-hoshu-hikaku/) | 昇降機の定期検査報告書・保守契約書を読み取って「EV保守 価格比較表」を作るツール。ブラウザで `index.html` を開いて使い、データは `▲0Claude` 配下の `data` フォルダに置く |

設定リポジトリと違い、ツールは `~/.claude` にリンクされません。
使うときはこのリポジトリのフォルダをそのまま開いてください。
