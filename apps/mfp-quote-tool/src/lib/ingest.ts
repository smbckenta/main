...
export async function ingestDocuments(
  ...
  if (!useAi && sawAiTarget) {
    // いちばん先頭に出す。ここを見落とすと、精度の落ちた読み取り結果のまま
    // 見積を作ってしまう
    warnings.unshift(
      settings.ai.enabled
        ? "【重要】AIのAPIキーが未設定のため、文字起こし（OCR）で読み取りました。" +
            "カウンター明細は枚数・単価を取りこぼします。設定画面でAPIキーを登録し、" +
            "もう一度読み取り直してください。"
        : "【重要】設定でAIの読み取りを「使わない」にしているため、文字起こし（OCR）で読み取りました。" +
            "カウンター明細は枚数・単価を取りこぼします。",
    );
  }
  if (!bestLease)
    warnings.push(
