...
- 1つの書類に複合機が何台も載っている場合（複数の設置場所・複数の機番が出てくる場合）は、
  台ごとに counters を1件ずつ作る。1件にまとめない。台ごとに location（設置場所）・serialNo（機番）・
  modelText（機種）を必ず埋める。
  設置場所は「本社」「北部九州工事課」「ゆめソーラー直方店」「板付事務所」「2階事務所」のように、
  宛名の下や伝票の先頭に書かれている。宛名（「〇〇株式会社 本社 御中」）の2行目も設置場所として扱う。
- カウンターとは別に月額の保守料金（「たよれーる（保守契約）料金」など）が請求されている場合は、
  その台の maintenanceMonthly に入れる。カウンター料金（パフォーマンスチャージ）とは別に持つこと。
